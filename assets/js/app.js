...
$(document).ready(function(){
	...
	var height = $(window).height();
		$('#bg-container, #background-1, #background-2').css({
			'min-width': width,
			'min-height': height
		});
...
var fade1 = function(){
	$('.background-1').velocity("fadeIn", { duration: 6000 });
	$('.background-2').velocity("fadeOut", { duration: 8000 });
};
// fade in bg2 and fade out bg1
var fade2 = function(){
	$('.background-2').velocity("fadeIn", { duration: 6000 });
	$('.background-1').velocity("fadeOut", { duration: 8000 });
};

// timeout function to create new svg every 10 seconds
var recreateSvg = function(){
 	window.setInterval(svgNew, 10000);
};
